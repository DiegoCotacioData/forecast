...
class ForecastPosprocessingStep:
    ...
    def create_rolling_forecast(self):
        ...
        df_forecast_rolling = df_forecast_rolling[["forecast_date","product_name","rolling_forecast"]]
    ...
    def create_global_metrics(self):
        ...
        # mode-product metrics
        #global_metrics = cv_metrics.groupby(['forecast_idx', 'product_name'])[['TFT-L_smape','TFT-S_smape', 'DeepAR_smape','N-Hits_smape', 'rolling_smape']].mean().reset_index()
        global_metrics = df_cv_metrics.groupby(['forecast_idx', 'product_name'])[['DeepAR_smape','N-Hits_smape', 'rolling_smape']].mean().reset_index()

        def calculate_best_smape_model(global_metrics):

            global_metrics = global_metrics.copy()
            grouped_data = global_metrics.groupby(['product_name'])

            def find_best_model(group):
                avg_smape = group[['DeepAR_smape', 'N-Hits_smape', 'rolling_smape']].mean()
            ...
            final_df = global_metrics.merge(result_df, on=['product_name'])
    ...
    def raw_forecast_creation(self, rolling_forecast, global_metrics):
        ...
        for model_name, model_output in self.dict_forecast_results.items():
            ...
            rollingx = model_output.merge(rollingx, on=['forecast_date', 'product_name'], how='left')
            ...
            raw_forecast_dataframe = pd.merge(rollingx, global_metrics, on=["forecast_idx","product_name"], how="left")
